    ...
    if request.method == 'POST':
        try:
            ...
            with sqlite3.connect('database.db') as con:
                cur = con.cursor()
                cur.execute("INSERT INTO sales (name, price, quantity, code) VALUES (?,?,?,?)",(nm, price, quantity, code))
...
def editrec():
    ...
    if request.method == 'POST':
        try:
            ...
            with sqlite3.connect('database.db') as con:
                cur = con.cursor()
                cur.execute("UPDATE sales SET name='"+nm+"', price='"+price+"', quantity='"+quantity+"', code='"+code+"' WHERE rowid="+rowid)

                con.commit()
                msg = "Record successfully edited in the database"
        except:
            con.rollback()
            msg = "Error in the Edit: UPDATE sales SET name="+nm+", price="+price+", quantity="+quantity+", code="+code+" WHERE rowid="+rowid
